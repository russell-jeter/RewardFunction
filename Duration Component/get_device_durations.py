...
for device in device_list:
    ...
    for date in dates:
        ...
        active_sum_list.append(date_active_sum)
        final_device_list.append(device)
        final_date_list.append(date)

device_active_time_list = []

for i in range(len(final_device_list)):
    device_active_time_list.append([final_device_list[i], final_date_list[i], active_sum_list[i], ])


column_list = ['device_id', 'date', 'active_time']
device_active_time_table = pd.DataFrame(device_active_time_list, columns = column_list)

device_active_time_table.to_csv('./data/device_active_times.txt', sep = '|', index = False)